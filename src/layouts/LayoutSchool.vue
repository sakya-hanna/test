<!--
LayoutSchool.vue — 学校管理端后台页面统一外壳
适用角色：学校管理员
-->
<script setup>
...
const menuItems = [
  { label: '工作台',       key: '工作台',       path: '/school/dashboard' },
  { label: '企业管理',     key: '企业管理',     path: '/school/companies/review' },
  { label: '实习管理',     key: '实习管理',     path: '/school/internship-plans' },
  { label: '通知公告',     key: '通知公告',     path: '/school/notifications' },
  { label: '统计分析',     key: '统计分析',     path: '/school/analytics' },
  { label: '消息中心',     key: '消息中心',     path: '/school/messages' },
]

const activeKey = computed(() => {
  const exact = menuItems.find(m => m.path === route.path)
  if (exact) return exact.key
  if (route.path.startsWith('/school/companies')) return '企业管理'
  if (route.path.startsWith('/school/internship')) return '实习管理'
  if (route.path.startsWith('/school/notifications')) return '通知公告'
  if (route.path.startsWith('/school/analytics')) return '统计分析'
  if (route.path.startsWith('/school/messages')) return '消息中心'
  return '工作台'
})
...
const userName = ref('王处长')
const userRole = ref('学校管理员')
</script>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      <header class="h-[72px] bg-white flex items-center justify-between px-[24px] shrink-0 border-b border-[#e5e7eb]">
        ...
        <div class="flex items-center gap-[12px]">
          <div class="w-[40px] h-[40px] rounded-full bg-[#d1d5db] flex items-center justify-center text-[14px] font-medium text-[#6b7280]">王</div>
          <div>
